...
class uiscreenSumming():
    def display():
        '''一个阶段（3回合）结束，打印章末结算页，返回总盈利'''
        info_linelists = []
        total_sum = 0

        print(f"                       第 {UserData.game_round} 回合结算                          ")
        print()
        print("     饮品     名称            体积(ml)       尺寸       单项盈利")
        #print()
        time.sleep(1)
        for bottle in UserData.user_bottles:
            ...
            chinese_name = color_phrase(bottle.tag)
        ...
        for line in info_linelists:
            for i in range(len(line[0])):
                ...
                if i == len(line[0]) // 2:
                    print(space_counter(line[0][i], 14, mode="middle") + space_counter(line[1], 18), space_counter(str(line[2]), 12), space_counter(line[3], 12),  space_counter(judgeEarnOrLose(line[4]), 8), end = "")
                else:
                    print(space_counter(line[0][i], 14, mode="middle"), end = "")
                print()
            time.sleep(0.5)

        time.sleep(0.5)
        print()
        print("   总盈利：                                               ", judgeEarnOrLose(total_sum))
        print()
        print()
        input(">>> 按回车来进行下一回合")
        return total_sum
